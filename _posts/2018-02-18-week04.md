...
### (As of the time this blog post is written, my pull request is not closed)
...
2) Fork the repo through Github on the [course website repository](https://github.com/joannakl/cs480_s18) and add it to my own GitHub account's repository
...
```
7) Fix the footer information in "footer.html"
...
$ git add footer.html
```
10) Commit my changes with a message:
```
$ git commit -m 'Corrected name and URL of footer')
```
11) I pushed the change to my fork
```
